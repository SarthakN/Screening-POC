...
import { findSimilarCriteriaJobs } from '../services/llm'
...
export default function AddJobForm({
  ...
  const handleSubmit = (e) => {
    ...
    if (reuseActive) {
      ...
      try {
        ...
        if (matches.length === 1) {
          addJobWithCriteria(draftJob, matches[0].criteria)
          return
        }
  ...
  const handleConfirmReuseChoice = () => {
    ...
    addJobWithCriteria(reuseChoices.draftJob, selectedMatch.criteria)
  }
  ...
  return (
    <form className="add-job-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="form-footer">
        <button type="submit" className="btn-primary" disabled={reusingCriteria}>
          {reusingCriteria ? 'Matching previous job...' : 'Add Job'}
        </button>
      </div>
    </form>
  )
